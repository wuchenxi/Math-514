...
\paragraph{Explicit Methods} Consider the function $g(t)=f(t, y(t))$. One can then use $g(t-h), \dots, g(t-kh)$ as quadrature point to estimate $\int_{t-dh}^{t}g(s)ds$, then use $g(t)=g(t-dh)+\int_{t-dh}^{t}g(s)ds$.

\paragraph{Implicit Methods} One can also use $g(t), g(t-h), \dots, g(t-kh)$ as quadrature point to estimate $\int_{t-dh}^{t}g(s)ds$, then use $g(t)=g(t-dh)+\int_{t-dh}^{t}g(s)ds$. This way $g(t)$ appears on the right hand side and the calculation of $g(t)$ requires solving an equation, hence is called {\bf implicit}.
