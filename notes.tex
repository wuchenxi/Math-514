...
\newpage
\item Now let's see how this error grow with time: Let $w_m=z_k((k+1+n)h)-z_{k+1}((k+1+m)h)$. Then
  \[w_0=0\]
  \[w_1=O(h^2)\]
...
\item To get consistency, let $\sigma(z)=\sum_{j=0}^k\beta_jz^j$ be the {\bf second characteristic polynomial}. When $f$ is smooth, $s<<1$, $f(nh+s, y(nh+s))=f(nh, y(nh))+O(s)$, $y(nh+s)=y(nh)+sf(nh, y(nh))+O(s^2)$. To make method consistent, we want to make sure that if we put $y((n+j)h)$ in place of $z((n+j)h)$, the left hand side and right hand side are off by $o(h)$, hence
  ...
  So $\rho(1)=\sum_i\alpha_i=0$, $\rho'(0)=\sum_ii\alpha_i=\sum_i\beta_i=\sigma(1)$.
...
\item (First Dahlquist barrier) If a linear $k$-step method is $k$ stable then the order of accuracy is no more than $k+1$ if $k$ is odd (e.g. Adams-Moulton, by Theorem~\ref{bound1}), $k+2$ if $k$ is even (e.g. $d=k$ implicit, by Theorem~\ref{qbound}), and $k$ if it has to be explicit (e.g. Adams-Bashforth, by Theorem~\ref{bound1}).
\end{itemize}
...
\[z((n+1)h)=z(nh)+\sum_jc_jk_j\]
Some popular choice of parameters:
\begin{enumerate}
\item Henu's method, or improved Euler method: $\alpha_2=\beta_{12}=1$, $c_1=c_2=1/2$
...
\begin{exa}Show that Henu's method is consistent and has order of accuracy $2$.\end{exa}
\begin{enumerate}
\item
  \[f(t+s, y(t)+r)=f(t, y(t))+sf_1(t, y(t))+rf_2(t, y(t))+O(r^2+s^2)\]
\item So, suppose $y(t+s)=y(t)+a_1s+a_2s^2+O(s^3)$, then $y'=f(t, y)$ implies
  \[a_1+2a_2s=f(t, y(t))+s((f_1+a_1f_2)(t, y(t)))+O(s^2)\]
  So $a_1=f(t, y(t))$, $a_2={f_1(t, y(t))+f(t, y(t))f_2(t, y(t))\over 2}$, and
  \[y(t+h)=y(t)+a_1h+a_2h^2+O(h^3)\]
\item Now do Henu's method:
  \[k_1=f(t, y(t))=a_1\]
  \[k_2=f(t+h, y(t)+hk_1)=a_1+hf_1+a_1hf_2+O(h^2)=a_1+2ha_2+O(h^2)\]
  So
  \[z(t+h)=y_t+a_1h+a_2h^2+O(h^3)=y(t+h)+O(h^3)\]
