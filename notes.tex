...
\item {\bf Truncated error} $T_n$ is the difference between the calculated $z((n+1)h)-z(nh)\over h$, and $y_n((n+1)h)-y_n(nh)\over h$, where $y_n$ is the solution for initial value $y_n(nh)=z(nh)$. 
...
\begin{tikzpicture}[yscale=2, xscale=8]
  ...
  \draw[scale=1, domain=0.25:1, smooth, variable=\x, red] plot({\x}, {(exp(\x-0.25)*1.25});
  
  \draw[scale=1, domain=0.5:1, smooth, variable=\x, red] plot({\x}, {(exp(\x-0.5)*1.25*1.25});
  
  \draw[scale=1, domain=0.75:1, smooth, variable=\x, red] plot({\x}, {(exp(\x-0.75)*1.25*1.25*1.25});
\end{tikzpicture}
