...
\item Globally: separate the error created at each step, as in the argument for Euler's method. We can summarize it as below:
  \begin{thm}(Theorem 12.2 in textbook)
    If $z((n+1)h)=z(nh)+\Phi(nh, z(nh))$, $z(0)=y(0)$, $|y((n+1)h)-\Phi(y(nh))|\leq Th$, and $\Phi$ is $hL$-Lispchitz with respect to the second parameter. Then
    \[|z(nh)-y(nh)|\leq T\cdot {e^{nhL}-1\over L}\]
 \end{thm}
 \begin{proof}
   Let $z_k(kh)=y(kh)$, $z_k((n+1)h)=z_k(nh)+\Phi(nh, z_k(nh))$, then
...
\item Consistency and order of accuracy:
  ...
  \[=y(t)+f(t, y(t))h+(f_1+ff_2){h^2\over 2}+(f_{11}+2f_{12}f+f_{22}f^2){h^3\over 6}+\dots\]
  So
  \[y(t+h)-z(t+h)=O(h^3)\]
