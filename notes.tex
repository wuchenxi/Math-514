...
\subsubsection{Modified Gauss Quadratures (Section 10.6)}
...
\subsubsection{Richardson Extrapolation (Section 7.6, 7.7)}
...
\[y'=f(t, y), y(0)=y_0\]
Here $y$ can be a real valued function or $\mathbb{R}^n$-valued function. We always assume $f$ is continuous and Lipschitz on the second parameter. (So by Picard's Theorem, IVP has a unique solution.)\\
...
\begin{itemize}
\item {\bf Global error at time} $t$ is the difference between the estimated $y(t)$ and the true value of $y(t)$.
\item {\bf Truncated error} $T$ is the difference between the calculated $y(t+h)-y(t)\over h$ and the true $y(t+h)-y(t)\over h$.
\item Under certain assumptions, global error at given time $t$ is controlled by the bound on truncated error (Theorem 12.2 in textbook). 
\item A method has {\bf order of accuracy} $p$ if when $f$ is sufficiently smooth, $T=O(h^p)$.
\end{itemize}

By Taylor expansion,
\[y(t+h)=y(t)+hf(t, y(t))+{h^2\over 2}y''(t)+\dots\]
So for Euler's method $T=O(h)$, the order of accuracy is $1$.
