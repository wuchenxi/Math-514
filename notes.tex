...
\item Globally: separate the error created at each step, as in the argument for Euler's method. We can summarize it as below:
  \begin{thm}\label{global}(Theorem 12.2 in textbook)
    If $z((n+1)h)=z(nh)+h\Phi(nh, z(nh))$, $z(0)=y(0)$, $|y((n+1)h)-h\Phi(y(nh))|\leq Th$, and $\Phi$ is $L$-Lispchitz with respect to the second parameter. Then
...
\begin{exa}Consider IVP $y'=f(y)$, $y(0)=y_0$, where $f, f^{(k)}$ are all bounded by $1$. Consider the linear multistep method
  ...
  \[z(t+2h)=z(t)+{hf(z(t))\over 3}+{4hf(z(t+h))\over 3}+{hf(z(t+2h))\over 3}\]
    ...
    \[|z(t+2h)-y(t+2h)|<Ch^5\]
    And find the domain of absolute stability for this method.
  \end{exa}

  Answer:
  \begin{itemize}
  \item Firstly, by error bound of Simpson's rule,
    \[y(t+2h)=y(t)+\int_0^{2h}f(y(s))ds\]
    \[=y(t)+{hf(y(t))\over 3}+{4hf(y(t+h))\over 3}+{hf(y(t+2h))\over 3}+E\]
    Here
    \[|E|\leq{(f\circ y)^{(4)}(2h)^5\over 2880}\]
    \[\leq {4h^5\over 45}\]
  \item Now, by assumption,
    \[|y(t+2h)-z(t+2h)|\leq h|y(t+2h)-z(t+2h)|+|E|\]
    So, for example, if $h<1/2$, we have
    \[|y(t+2h)-z(t+2h)|\leq {8\over 45}h^5\]
  \item Apply the method to $y'=\lambda y$, we get
    \[z(t+2h)=z(t)+{h\lambda z(t)\over 3}+{4h\lambda z(t+h)\over 3}+{h\lambda z(t+2h)\over 3}\]
    So absolute stability implies
    \[(1-h\lambda/3)z^2-(4h\lambda/3)-(1+h\lambda/3)\]
    has all roots inside the unit circle.
  \end{itemize}
