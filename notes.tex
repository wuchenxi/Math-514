...
\item Consistency and order of accuracy:
  ...
  \[z(t+h)=y(t)+hf(t+{h\over 2}, y(t)+{h\over 2}f(t, z(t)))\]
...
\item $\Phi(t, y)=f(t+{h\over 2}, z(t)+{h\over 2}f(t, z(t)))$, hence it is $L+hL^2$-Lipschitz.
\item Hence in this case, by Theorem~\ref{global}, 
  \[|y(t)-z(t)|\leq Kh^2(e^{(L+hL^2)t}-1)=O(h^2)\]
\end{enumerate}
...
\item Find $A$, $B$ such that the linear multistep method
  ...
  is consistent.
\item Find $A$, $B$ such that the linear multistep method has order of accuracy $2$.

\newpage

\item Estimate $y(h)$ using Euler's method, then $y(2h)$ using this linear multistep method.
\item Find the region of absolute stability of this linear multistep method, using the fact (from linear algebra) that iterative relation
  \[a_{n+2}=ca_{n+1}+da_n\]
  has all solutions converging to $0$ iff all roots of $z^2-(cz+d)$ has absolute value less than $1$. 
\end{enumerate}
