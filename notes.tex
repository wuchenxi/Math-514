...
\[y(t+h)=y(t)+hf(t, y(t))+{h^2\over 2}y''(t)+o(n^2)\]
...
\begin{lem}
  If $f$ is $L$-Lip. on second paramater, $z_1$, $z_2$ two functions defined on $mh, (m+1)h, \dots, nh$, such that
  \[z_i((k+1)h)=z_i(kh)+hf(kh, z_i(kh))\]
  Then 
\[|z_1(nh)-z_2(nh)|\leq e^{L(n-m)h}|z_1(mh)-z_2(mh)|\]
\end{lem}

\begin{proof}
  \[LHS\leq (1+hL)|z_1((n-1)h)-z_2((n-1)h)|\leq\]
    \[\dots\leq (1+hL)^{n-m}|z_1(mh)-z_2(mh)|\]
