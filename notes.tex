...
\[|y(nh)-z(nh)|=|z_{n}(nh)-z_{0}(nh)|\leq \sum_{k=0}^{n-1}|z_k(nh)-z_{k+1}(nh)|\leq \sum_{k=0}^{n-1}e^{Lh(n-k-1)}|z_k((k+1)h)-z_{k+1}((k+1)h)|\]
...
\item Hence, the Gauss-Legendre quadrature with $n=0$ is $x_0={b+a\over 2}$. Quadrature weight is ${b-a\over 2}$.
\item If $f$ is differentiable, the error bound is
  \[|\int_a^bfdx-{b-a\over 2}f({a+b\over 2})|\leq {\max|f''|\int_a^b(x-{a+b\over 2})^2\over 2!}={\max|f''|(b-a)^3\over 24}\]
...
\item Let $t=1$, $h=0.1$, Euler's method get $z(1)=1.95109$. True answer is $y(1)=1.95629$. Error bound as calculated from above is $0.08169$.
\end{enumerate}
