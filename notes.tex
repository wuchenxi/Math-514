...
(3.4011973816621555,5.530534801579673)
   ...
   \item Interpolation
    ...
    \item Approximation theory
      ...
      \item We can find an alternative error estimate via the following procedure
        \begin{enumerate}
         ...
         \item Integrate the error estimate, let $c\rightarrow {\pi/2}$, we get
           \[|f(x)-I(f)|\leq {1\over 24}\int_0^\pi |x(x-\pi/2)^2(x-\pi)| dx={\pi^4\over 2880}\approx 0.106\]
      ...
      % Exercises 1:\\
      % \begin{itemize}
      % \item $p=c^3(1-c)\cdot {x(x-1))\over c(c-1)}=-cx(x-1)$.
      % \item $I=c/6$. 
      % \item $\int_0^1f(x)dx=1/20$, so $c$ should be $3/10$.
      % \end{itemize}
