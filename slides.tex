...
\begin{proof}
  ...
  Then by calculation, $p(x_i)=y_i$, $p'(x_i)=z_i$.
  \end{proof}
...
\begin{thm}Let $n$ be an even number, $f\in C^{n+2}([a, b])$, $I_n(f)$ the Newton-Cotes formula using $n+1$ evenly spaced points on $[a, b]$, then there is some $C_n$ (depending on $n$) such that
...
For the same reason as in Example~\ref{exa2}, when $n\rightarrow\infty$ the error can not be guaranteed to decay to $0$. So we often evenly decompose the interval $[a, b]$ into subintervals then carry out low order Newton-Cotes. For example, if we cut it into $n$ subintervals and apply trapezium rule on each we get
\[{b-a\over n}(f(x_0)/2+\sum_{i=1}^{n-1}f(x_i)+f(x_n)/2)\]
If we cut it into $n/2$ subintervals, and apply simpson's rule on each, we get
\[{b-a\over 3n}(f(x_0)+4\sum_{i=1}^{n/2}f(x_{2i-1})+2\sum_{i=1}^{n/2-1}f(x_{2i})+f(x_n)\]

The error estimate can be calculated as before. When $f$ is smooth the error decay like $O(n^{-3})$ and $O(n^{-5})$ respectively.
...
\begin{verbatim}
2 0.9330127018922193 -5.405144181208228
3 0.9460173327169924 -4.139270839443451
4 0.9506292692220368 -3.692553073204491
5 0.952775736166314 -3.456730150933653
6 0.9539450135525174 -3.3079228288736666
7 0.9546512047359057 -3.2039874286754015
8 0.9551100227450633 -3.1264744308706844
9 0.9554248044017013 -3.0659595201887435
10 0.9556500753040448 -3.017094894510029
4 0.9565014583319759 -6.574978802776244
6 0.9565875738310257 -5.939122508722478
8 0.9566036072527387 -5.651720448514391
10 0.9566081883499551 -5.482917566334656
12 0.9566098729686051 -5.369491332969493
14 0.9566106052007204 -5.286790845669449
16 0.9566109637450405 -5.223110718777519
18 0.9566111557047383 -5.172130454182419
20 0.9566112658643311 -5.13011276845817
\end{verbatim}

If $n$ gets even higher the rounding error will have more prominent effect.

% \subsection{Extrapolation}

% A way to improve on the performance of the composite method above is via ``extrapolation''. \\

% Suppose $f\in C^\infty([a, b])$, $[a, b]$ evenly decomposed into $n$ subintervals, each with size $h$. Now consider the function
% \[F_i(t)=\int_0^t (f(x_{i-1}+t)+f(x_i-t))dt\]
% Then, $\int_a^bf(x)dx=\sum_iF_i(h/2)$.
% Now do Talyor series expansion for $F_i$, we get:
% \[F_i(h/2)={h\over 2}(f(x_{i-1})+f(x_i))+{h^2\over 8}(f'(x_{x-1})-f'(x_i))+\dots\]

% \newpage

% \begin{exa}Calculating $\int_0^1\sqrt{1-x^2} dx$. (The answer is $\pi/4$, so this is essentially calculating $\pi$.)

% \end{exa}
  
% \subsection{Gauss rules}

% \section{Initial Value Problem (Chapter 12)}
